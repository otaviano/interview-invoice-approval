---
name: invoice-approval-patterns
description: Enforces project architecture, conventions, and code patterns for the InvoiceApproval .NET solution. Use when creating new features, endpoints, use cases, domain services, tests, or modifying existing code in this project.
---
...
Clean Architecture with CQRS (MediatR). No database – all logic is in-memory.

```
src/
├── InvoiceApproval.Api           → Endpoints, ViewModels, Filters, Middlewares
├── InvoiceApproval.Application   → UseCases (Commands, Handlers, Validators, Results)
...
**Dependency rules:** Domain → nothing | Application → Domain | Infra.IoC → Application, Domain | Api → Infra.Core, Infra.IoC
...
**Handler** – `sealed class` with primary constructor, inject `IValidator<T>` + domain service:
...
| DI extension | `Add{Category}` | `AddDomainServices`, `AddUseCases` |
...
- **Always fail fast** — use early returns / guard clauses instead of nested `if` blocks. Check the negative condition, handle it (or return), and keep the main logic at the top indentation level
...
public class FeatureNameCommandHandlerTests
{
    ...
    {
        _sut = new FeatureNameCommandHandler(_validator, _myService);
    }
